...
export const useInventoryWithEndingStock = () => {
  const fetchInventoryWithEndingStock = async (): Promise<
  ...
  > => {
    ...
    try {
      ...
      return (spareParts as SparePart[]).map(
        (part): SparePartWithEndingStock => {
      ...
      console.warn(
        "Failed to calculate ending stock, using current stock:",
        calcError,
      );
      return (spareParts as SparePart[]).map(
        (part): SparePartWithEndingStock => ({
