...
export async function getSalesAnalytics(
  ...
  try {
    ...
    const vehicleIds =
      orders?.map((order) => order.vehicle?.id).filter(Boolean) || [];
...
> {
  try {
    ...
    const totalValue =
      spareParts?.reduce(
        (sum, part) => sum + part.price * part.stock_quantity,
        0,
      ) || 0;
    const lowStockItems =
      spareParts?.filter(
        (part) => part.stock_quantity <= 5 && part.stock_quantity > 0,
      ).length || 0;
    const outOfStockItems =
      spareParts?.filter((part) => part.stock_quantity === 0).length || 0;
...
export async function getSalesReport(
  ...
  try {
    ...
    const brandStats =
      orders?.reduce(
        (acc: Record<string, { count: number; amount: number }>, order) => {
          const brand = order.vehicle?.brand || "Unknown";
          const vehicleId = order.vehicle?.id;
    ...
    const reportOrders = Object.entries(brandStats)
      .map(
        (
          [vehicleBrand, stats]: [string, { count: number; amount: number }],
          index,
        ) => ({
          stt: index + 1,
          vehicleBrand,
          ...
          amount: stats.amount,
          rate: totalRevenue > 0 ? (stats.amount / totalRevenue) * 100 : 0,
        }),
      )
