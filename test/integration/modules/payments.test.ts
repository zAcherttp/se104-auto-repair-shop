...
import { createTestUser, createTestVehicle, createTestRepairOrder } from '../fixtures/factories';
...
describe('Payment Integration', () => {
  ...
  describe('Payment Creation and Vehicle Total Paid Update', () => {
    ...
    it('tracks different payment methods', async () => {
      ...
      const transferPayment = payments?.find(p => p.payment_method === 'transfer');
  ...
  describe('Debt Calculation', () => {
    it('calculates remaining debt correctly', async () => {
      ...
      const totalRepairCosts = vehicle?.repair_orders?.reduce((sum: number, order: any) => 
        sum + (order.total_amount || 0), 0) || 0;
      const totalPaid = vehicle?.payments?.reduce((sum: number, payment: any) => 
