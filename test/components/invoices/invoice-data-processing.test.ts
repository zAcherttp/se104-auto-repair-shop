...
describe("Invoice/Payments Data Processing", () => {
  ...
  describe("Profile Information Processing", () => {
    const getCreatedByDisplayName = (profile?: {
    ...
    const getCreatedByEmail = (profile?: {
      full_name: string | null;
      email: string;
    }): string => {
      if (!profile) return "";
    ...
    const formatCreatedByInfo = (profile?: {
      full_name: string | null;
      email: string;
    }) => {
    ...
    it("handles profile with no full name", () => {
      const info = formatCreatedByInfo(
        mockPaymentDataMissingInfo.created_by_profile,
      );
    ...
    it("handles undefined profile", () => {
      const info = formatCreatedByInfo(
        mockPaymentDataNoProfile.created_by_profile,
      );
      expect(info.displayName).toBe("Unknown");
