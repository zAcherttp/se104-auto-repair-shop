...
describe("Track Order Calculation Logic", () => {
  describe("Total Expense Calculations", () => {
    ...
    it("calculates multiple repair orders total correctly", () => {
      const totalExpense =
        mockOrderDataMultipleOrders.RepairOrderWithItemsDetails.reduce(
          (sum, order) => sum + (order.total_amount || 0),
          0,
        );
  ...
  describe("Payment Total Calculations", () => {
    it("calculates total payments correctly", () => {
      const totalPaid =
        mockOrderData.vehicle.payments?.reduce(
    ...
    it("calculates large payment amounts correctly", () => {
      const largePayments = [{ amount: 999999.99 }, { amount: 0.01 }];
      ...
      expect(totalPaid).toBe(1000000.0);
    });
  });
  ...
  describe("Remaining Amount Calculations", () => {
    it("calculates outstanding debt correctly", () => {
      const totalExpense =
        mockOrderDataWithDebt.RepairOrderWithItemsDetails.reduce(
          (sum, order) => sum + (order.total_amount || 0),
          0,
        );
      const totalPaid =
        mockOrderDataWithDebt.vehicle.payments?.reduce(
          (sum, payment) => sum + payment.amount,
          0,
        ) || 0;
      ...
      expect(remainingAmount).toBe(345.0);
      expect(remainingAmount > 0).toBe(true); // Should be positive (debt)
    });

    it("calculates overpayment correctly", () => {
      const totalExpense =
        mockOrderDataOverpaid.RepairOrderWithItemsDetails.reduce(
          (sum, order) => sum + (order.total_amount || 0),
          0,
        );
      const totalPaid =
        mockOrderDataOverpaid.vehicle.payments?.reduce(
          (sum, payment) => sum + payment.amount,
          0,
        ) || 0;
      const remainingAmount = totalExpense - totalPaid;
      ...
      expect(remainingAmount).toBe(-105.0);
      expect(remainingAmount < 0).toBe(true); // Should be negative (overpaid)
    });

    it("calculates exact payment correctly", () => {
      const totalExpense =
        mockOrderDataPaidInFull.RepairOrderWithItemsDetails.reduce(
          (sum, order) => sum + (order.total_amount || 0),
          0,
        );
      const totalPaid =
        mockOrderDataPaidInFull.vehicle.payments?.reduce(
          (sum, payment) => sum + payment.amount,
          0,
        ) || 0;
      const remainingAmount = totalExpense - totalPaid;
  ...
  describe("Repair Order Item Calculations", () => {
    it("validates individual item totals", () => {
      ...
      const expectedTotal =
        (firstItem.unit_price || 0) * (firstItem.quantity || 0) +
        (firstItem.labor_cost || 0);

      // (150 * 2) + 160 = 460
      expect(firstItem.total_amount).toBe(expectedTotal);
      expect(firstItem.total_amount).toBe(460.0);
    });
    ...
    it("validates item totals sum to order total", () => {
      ...
      expect(itemsTotal).toBe(545.0); // 460 + 85
    });
    ...
    it("handles zero quantities correctly", () => {
      ...
      const expectedTotal =
        mockItemZeroQuantity.unit_price * mockItemZeroQuantity.quantity +
        mockItemZeroQuantity.labor_cost;
      expect(expectedTotal).toBe(50.0); // 0 + 50
    });
    ...
    it("handles zero labor cost correctly", () => {
      ...
      const expectedTotal =
        mockItemZeroLabor.unit_price * mockItemZeroLabor.quantity +
        mockItemZeroLabor.labor_cost;
      expect(expectedTotal).toBe(200.0); // 200 + 0
    });
  });
  ...
  describe("Business Logic Edge Cases", () => {
    it("handles scenario with no payments but has orders", () => {
      ...
      const totalPaid =
        mockNoPayments.vehicle.payments?.reduce(
          (sum, payment) => sum + payment.amount,
          0,
        ) || 0;
      const remainingAmount = totalExpense - totalPaid;
      ...
      expect(totalExpense).toBe(545.0);
      expect(totalPaid).toBe(0);
      expect(remainingAmount).toBe(545.0);
    });
    ...
    it("handles scenario with payments but no orders", () => {
      ...
      const totalPaid =
        mockNoOrders.vehicle.payments?.reduce(
          (sum, payment) => sum + payment.amount,
          0,
        ) || 0;
      const remainingAmount = totalExpense - totalPaid;
      ...
      expect(totalExpense).toBe(0);
      expect(totalPaid).toBe(350.0);
      expect(remainingAmount).toBe(-350.0); // All payments are overpayment
    });
    ...
    it("handles large numbers correctly", () => {
      ...
      expect(calculation).toBe(1000000000.0);
    });
