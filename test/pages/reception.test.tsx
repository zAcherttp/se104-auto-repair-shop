...
import { mockVehicleRegistrationData, mockVehicleLimit } from "@/test/mocks/reception-data";
...
jest.mock("@/components/reception/reception-form", () => ({
  ReceptionForm: jest.fn(({ open, onClose, onSuccess }) => (
    <div data-testid="reception-form" style={{ display: open ? "block" : "none" }}>
...
describe("Reception Page", () => {
  ...
  it("passes correct props to VehicleDataTable", () => {
    ...
    const { VehicleDataTable } = require("@/app/(protected)/reception/data-table");
  ...
  it("renders page wrapper with correct styling", () => {
    ...
    expect(wrapper).toHaveClass("w-full", "p-6");
  });
});
